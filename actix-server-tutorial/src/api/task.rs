...
//2. tie a handler function to path (want to handler function to receive requests)
//3. specify any http method that i want for handler to handle

//handler function should return a thing among two types
//1. a struct implements Responder trait
//2. Result for which ths success value implements responder traint and Error implements Response error
...
//implementation actix response error for Task error(overload)
...
//web::Path<T> is one of built-in actix extractors
//Just passing by struct as a type of Path (and also it can be deserialzed by serde), actix extractors can extract
//Btw, Because we used Path extractor, actix will extract it from path.
// ex) http://127.0.0.1:8080/task/12 -> task_global_id = 12
//If you want to extract data from body of json, Use web::Json<T> extractor
